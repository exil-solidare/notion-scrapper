...
function toPlainProperties(properties: PageObjectResponse['properties']) {
  const types = {
    title(prop: Extract<PageObjectResponse['properties'][string], { type: 'title' }>) {
      return prop.title[0]?.plain_text ?? null
    },
    rich_text(prop: Extract<PageObjectResponse['properties'][string], { type: 'rich_text' }>) {
      return prop.rich_text[0]?.plain_text
    },
    number(prop: Extract<PageObjectResponse['properties'][number], { type: 'number' }>) {
      return prop.number
    },
    select(prop: Extract<PageObjectResponse['properties'][string], { type: 'select' }>) {
      return prop.select?.name
    },
    multi_select(
      prop: Extract<PageObjectResponse['properties'][Array<T>], { type: 'multi_select' }>
    ) {
      return prop.multi_select.map((s) => s.name)
    },
    date(prop: Extract<PageObjectResponse['properties'][string], { type: 'date' }>) {
      return prop.date?.start ? new Date(prop.date?.start) : null
    },
    files(prop: Extract<PageObjectResponse['properties'][string], { type: 'files' }>) {
